...
class SchemaValidator:
    ...
    def _reconcile_columns_via_llm(
        self,
        missing_columns: List[str],
        extra_columns: List[str],
    ) -> List[Dict[str, str]]:
        """
        Use an LLM to find semantically equivalent column pairs between
        columns that appear to be 'dropped' (missing) and 'added' (extra).

        For example, the LLM preprocessor might name a column 'period' in one
        run and 'fiscal_year' in the next.  This method detects such renames so
        they don't break incremental-load detection.

        Args:
            missing_columns: Columns in the existing table but absent from the new file.
            extra_columns:   Columns in the new file but absent from the existing table.

        Returns:
            List of {"existing": <col>, "new": <col>} dicts for reconciled pairs.
        """
        if not missing_columns or not extra_columns:
            return []

        prompt = f"""You are a data-schema expert. Two datasets represent the same
data series but their column names differ slightly because they were
generated by automated preprocessing in separate runs.

Below are two lists:
  EXISTING columns (present in the database table but NOT in the new file):
    {json.dumps(missing_columns)}
  NEW columns (present in the new file but NOT in the database table):
    {json.dumps(extra_columns)}

Identify pairs of columns (one from each list) that are **semantically
equivalent** — i.e. they represent the same concept/measure despite different
naming.  Only include pairs you are confident about.

Respond ONLY with a JSON object:
{{"pairs": [{{"existing": "<existing_col>", "new": "<new_col>"}}, ...]}}

If no columns are semantically equivalent, return: {{"pairs": []}}"""
        ...
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are a column-name reconciliation agent. "
            ...
            # Validate: both sides must actually exist in the provided lists
            missing_set = set(missing_columns)
            extra_set = set(extra_columns)
            used_missing, used_extra = set(), set()
            validated: List[Dict[str, str]] = []

            for pair in raw_pairs:
                existing_col = pair.get("existing", "")
                new_col = pair.get("new", "")
                if (
                    existing_col in missing_set
                    and new_col in extra_set
                    and existing_col not in used_missing
                    and new_col not in used_extra
                ):
                    validated.append({"existing": existing_col, "new": new_col})
                    used_missing.add(existing_col)
                    used_extra.add(new_col)
                else:
                    logger.warning(
                        f"LLM reconciliation: discarding invalid pair "
                        f"'{existing_col}' ↔ '{new_col}'"
                    )

            logger.info(
                f"LLM column reconciliation: {len(validated)} pair(s) matched "
                f"out of {len(raw_pairs)} returned"
            )
            return validated

        except Exception as e:
            logger.error(f"LLM column reconciliation failed: {e} — returning no pairs")
            return []
    ...
    def validate_schema_match(
        ...
        # Calculate raw match percentage (before reconciliation)
        if len(existing_set) > 0:
            raw_match_percentage = (len(matching_columns) / len(existing_set)) * 100
        else:
            raw_match_percentage = 0.0

        # --- LLM column reconciliation ---
        # When both missing and extra columns exist, ask the LLM whether any
        # are semantically equivalent (e.g. 'period' ↔ 'fiscal_year').
        reconciled_columns: List[Dict[str, str]] = []
        if missing_columns and extra_columns:
            logger.info(
                f"Attempting LLM column reconciliation: "
                f"{len(missing_columns)} missing, {len(extra_columns)} extra"
            )
            reconciled_columns = self._reconcile_columns_via_llm(
                missing_columns, extra_columns
            )
            # Remove reconciled pairs from missing/extra lists
            for pair in reconciled_columns:
                if pair["existing"] in missing_columns:
                    missing_columns.remove(pair["existing"])
                if pair["new"] in extra_columns:
                    extra_columns.remove(pair["new"])
                logger.info(
                    f"  Reconciled: '{pair['existing']}' (existing) "
                    f"↔ '{pair['new']}' (new file)"
                )

        # Final match percentage includes reconciled columns
        effective_matches = len(matching_columns) + len(reconciled_columns)
        if len(existing_set) > 0:
            match_percentage = (effective_matches / len(existing_set)) * 100
        else:
            match_percentage = 0.0

        if reconciled_columns:
            logger.info(
                f"Match percentage: {raw_match_percentage:.2f}% (raw) → "
                f"{match_percentage:.2f}% (after reconciliation)"
            )
    ...
    def generate_discrepancy_report(
        ...
        # Show reconciled columns if any
        reconciled = validation_result.get('reconciled_columns', [])
        if reconciled:
            report_lines.append(f"Reconciled Columns ({len(reconciled)}) [LLM-MATCHED]:")
            report_lines.append("  Semantically equivalent column pairs:")
            for pair in reconciled:
                report_lines.append(
                    f"    ↔ '{pair['existing']}' (existing) = '{pair['new']}' (new file)"
                )
            report_lines.append("")
